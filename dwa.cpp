...
int max_ang_acceleration,length=5,bredth=5;
...
{
	public:
		PathPlanner(Point star, Point en, Mat ma){
			start = star;
			
			end = en;
			map = ma; 
			max_acceleration=8;
			max_ang_acceleration = 40;
		}
		void getPath(){
			curr_state.vel = 0;
			...
			while(!(curr_state.x==end.x&&curr_state.y==end.y)){
				...
				for(i=velocity+max_acceleration/4;i>=velocity-max_acceleration/4;i--){
					//cout << velocity << endl;
					if(i==0||abs(i)>25||i<0) continue;

					for(j=ang_velocity-max_ang_acceleration/4;j<=ang_velocity+max_ang_acceleration/4;j++){
						//cout << j << endl;
						...
						for(float t=0;true;t+=0.25*sgn(j)){
							...
							if(!isValid(x1,y1,theta,length,bredth)) {
								...
								dist = r * (t) * (3.14/180); 
								//cout << dist << "oh" << t <<endl;
								if(abs(i)<=sqrt(max_acceleration*dist*sgn(dist))-10){
...
int main(int argc,char** argv) {
	...
	PathPlanner dvij(start,end,image);
	dvij.getPath();
	cout << "Hurrah!!!!!!!!!!!!!!!!!!" <<endl;
}